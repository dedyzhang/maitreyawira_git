    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-4 flex items-center justify-between flex-wrap gap-3">
            <div class="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-3 flex-wrap">
                ...
                @if($classroom->semester)<span>ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Semester {{ $classroom->semester->semester }} {{ $classroom->semester->tahun }}</span>@endif
                <span>ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· @foreach($classroom->kelas as $k)<span class="font-medium">{{ $k->tingkat }}{{ $k->kelas }}</span>@if(!$loop->last), @endif @endforeach</span>
                <span>ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {{ $classroom->members->count() }} siswa</span>
            </div>
            @if($classroom->rombel)
        ...
        <a href="{{ route('classroom.assignment.show', [$a, 'class' => $classroom->uuid]) }}" class="card p-4 flex items-center gap-3 hover:border-primary transition">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if($a->status==='draft')<span class="text-[11px] px-2 py-0.5 rounded bg-amber-100 text-amber-700">Draf</span>@if($canManage)<span role="button" @click.prevent="fetch('{{ route('classroom.assignment.publish', $a) }}', {method: 'POST', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>window.location.reload())" class="text-[11px] px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition">Terbitkan</span>@endif @endif
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 truncate">{{ $a->title }}</h3>
                </div>
                <p class="text-xs text-slate-400 mt-0.5">Nilai maks {{ $a->max_score }}@if($a->due_at) ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· Batas {{ $a->due_at->locale('id')->translatedFormat('d M Y H:i') }}@endif
                    @if($me->access==='siswa') @php $sub=$mySubmissions[$a->uuid]??null; @endphp ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· @if($sub && $sub->status==='graded')<span class="text-emerald-600 font-semibold">@if($a->hide_scores)Tugas sudah dikoreksi@elseNilai {{ $sub->score }}@endif</span>@elseif($sub)<span class="text-sky-600">Sudah dikumpulkan</span>@else<span class="text-amber-600">Belum dikumpulkan</span>@endif @endif
                </p>
            </div>
            <div class="flex items-center gap-3 text-xs text-slate-400 flex-shrink-0">
