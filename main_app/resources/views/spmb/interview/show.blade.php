...
@section('container')
    <div class="p-4 bg-white rounded-lg shadow-md">
        ...
        <div class="mt-10">
            <a class="button-d bg-green-400 hover:bg-green-600 focus:ring-green-400 w-full block sm:inline sm:w-auto mb-1" href="{{ route('spmb.cetak.interview',$sekolah->uuid) }}">Cetak Hasil Interview</a>
        </div>
    </div>
    @if(session('success'))
            ...
            <table class="w-full text-sm text-gray-500 text-left rtl:text-right text-gray-500" id="table-sekolah">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3"><span class="flex items-center">Status <svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                        </svg></span></th>
                ...
                <tbody>
                    @foreach ($spmb_siswa as $siswa)
                        @php
                            ...
                            $interview = unserialize($siswa->interview);
                            if(isset($interview) && $interview != null) {
                                $class = "text-green-600";
                                $status_interview = "Sudah Mengisi Interview";
                            } else {
                                $class = "text-red-800";
                                $status_interview = "Belum Mengisi Interview";
                            }
                        @endphp
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-3"><i class="{{ $class }}">{{ $status_interview }}</i></td>
                            <td class="px-6 py-3 gap-1 flex">
                                <a href="{{ route('spmb.interview.edit',$siswa->uuid) }}" class="px-2 py-2 rounded-lg bg-blue-500 hover:bg-blue-700 focus:outline-0 focus:ring-5 focus:ring-blue-500 text-white"><span class="material-icons-round" style="font-size:16px">edit</span></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
